...
export const ListCard: React.FC<ListCardProps> = ({
 ...
 const isOwner = seznam.members.some(member => member.email === currentUserEmail && member.isOwner);
   ...
   <Card className="bg-white">
       ...
       <div className="menu-area">
         <DropdownMenu>
           ...
           <Button 
              ...
              data-testid="menu-button"  // Přidáno toto
              >
           ...
           <DropdownMenuContent align="end" className="w-48 bg-white border-[#7d9b69]">
             {isOwner && !seznam.archivovano && (
               <>
                 <DropdownMenuItem onClick={() => onEdit(seznam.id)} className="flex items-center px-3 py-2 text-white bg-[#7d9b69] hover:bg-[#4e6a4d]">
                   <Edit2 className="mr-2 h-4 w-4" />
                   Upravit
                 </DropdownMenuItem>
                 <DropdownMenuItem onClick={() => onShare(seznam.id)} className="flex items-center px-3 py-2 text-white bg-[#7d9b69] hover:bg-[#4e6a4d] mt-1">
                   <Share className="mr-2 h-4 w-4" />
                   Sdílet
                 </DropdownMenuItem>
                 <DropdownMenuItem onClick={() => setShowArchiveDialog(true)} className="flex items-center px-3 py-2 text-white bg-[#7d9b69] hover:bg-[#4e6a4d] mt-1">
             ...
             {isOwner && seznam.archivovano && (
               <DropdownMenuItem onClick={() => onUnarchive(seznam.id)} className="flex items-center px-3 py-2 text-white bg-[#7d9b69] hover:bg-[#4e6a4d]">
                 <X className="mr-2 h-4 w-4" />
                 Zrušit archivaci
               </DropdownMenuItem>
             )}
             {isOwner && (
               <DropdownMenuItem onClick={() => setShowDeleteDialog(true)} className="flex items-center px-3 py-2 text-white bg-red-500 hover:bg-red-600 mt-1">
     ...
     <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
       <AlertDialogContent className="bg-[#f3f8e8] border-[#7d9b69]">
         ...
         <AlertDialogFooter>
           <AlertDialogCancel className="border-[#7d9b69] text-[#4e6a4d] hover:bg-[#e2ebd3]">
             Zrušit
           </AlertDialogCancel>
           <AlertDialogAction
             onClick={() => {
